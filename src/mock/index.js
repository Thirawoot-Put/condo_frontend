export const popularAreas = [
  {
    image: 'https://photos.wikimapia.org/p/00/02/43/70/81_big.jpg',
    districtName: 'id',
    districtDetail: 'ipsum primis',
  },
  {
    image: 'https://photos.wikimapia.org/p/00/02/43/70/81_big.jpg',
    districtName: 'magna',
    districtDetail: 'tincidunt eu felis',
  },
  {
    image: 'https://photos.wikimapia.org/p/00/02/43/70/81_big.jpg',
    districtName: 'amet',
    districtDetail: 'tempus vivamus',
  },
  {
    image: 'https://photos.wikimapia.org/p/00/02/43/70/81_big.jpg',
    districtName: 'elit',
    districtDetail: 'nec sem duis',
  },
  {
    image: 'https://photos.wikimapia.org/p/00/02/43/70/81_big.jpg',
    districtName: 'lectus',
    districtDetail: 'faucibus orci luctus',
  },
  {
    image: 'https://photos.wikimapia.org/p/00/02/43/70/81_big.jpg',
    districtName: 'at',
    districtDetail: 'at nunc commodo',
  },
  {
    image: 'https://photos.wikimapia.org/p/00/02/43/70/81_big.jpg',
    districtName: 'at',
    districtDetail: 'quis orci',
  },
  {
    image: 'https://photos.wikimapia.org/p/00/02/43/70/81_big.jpg',
    districtName: 'in',
    districtDetail: 'aliquet massa',
  },
  {
    image: 'https://photos.wikimapia.org/p/00/02/43/70/81_big.jpg',
    districtName: 'donec',
    districtDetail: 'molestie nibh in',
  },
  {
    image: 'https://photos.wikimapia.org/p/00/02/43/70/81_big.jpg',
    districtName: 'aliquam',
    districtDetail: 'in lectus pellentesque',
  },
  {
    image: 'https://photos.wikimapia.org/p/00/02/43/70/81_big.jpg',
    districtName: 'sapien',
    districtDetail: 'porttitor lacus at',
  },
  {
    image: 'https://photos.wikimapia.org/p/00/02/43/70/81_big.jpg',
    districtName: 'nec',
    districtDetail: 'ut nulla',
  },
  {
    image: 'https://photos.wikimapia.org/p/00/02/43/70/81_big.jpg',
    districtName: 'sed',
    districtDetail: 'platea dictumst etiam',
  },
  {
    image: 'https://photos.wikimapia.org/p/00/02/43/70/81_big.jpg',
    districtName: 'nascetur',
    districtDetail: 'dapibus augue vel',
  },
  {
    image: 'https://photos.wikimapia.org/p/00/02/43/70/81_big.jpg',
    districtName: 'nibh',
    districtDetail: 'et magnis',
  },
  {
    image: 'https://photos.wikimapia.org/p/00/02/43/70/81_big.jpg',
    districtName: 'at',
    districtDetail: 'mauris ullamcorper purus',
  },
  {
    image: 'https://photos.wikimapia.org/p/00/02/43/70/81_big.jpg',
    districtName: 'diam',
    districtDetail: 'mauris laoreet',
  },
  {
    image: 'https://photos.wikimapia.org/p/00/02/43/70/81_big.jpg',
    districtName: 'lorem',
    districtDetail: 'suspendisse potenti in',
  },
  {
    image: 'https://photos.wikimapia.org/p/00/02/43/70/81_big.jpg',
    districtName: 'porttitor',
    districtDetail: 'interdum venenatis',
  },
  {
    image: 'https://photos.wikimapia.org/p/00/02/43/70/81_big.jpg',
    districtName: 'amet',
    districtDetail: 'eget tincidunt eget',
  },
];

export const featuredLists = [
  {
    id: 1,
    img: 'https://www.99acres.com/microsite/articles/files/2023/02/condo.jpg',
    roomDetail: '111/A',
    condoName: 'Condo name',
    price: '999',
  },
  {
    id: 2,
    img: 'https://www.99acres.com/microsite/articles/files/2023/02/condo.jpg',
    roomDetail: '222/A',
    condoName: 'Condo name',
    price: '999',
  },
  {
    id: 3,
    img: 'https://www.99acres.com/microsite/articles/files/2023/02/condo.jpg',
    roomDetail: '333/A',
    condoName: 'Condo name',
    price: '999',
  },
  {
    id: 4,
    img: 'https://www.99acres.com/microsite/articles/files/2023/02/condo.jpg',
    roomDetail: '444/A',
    condoName: 'Condo name',
    price: '999',
  },
  {
    id: 5,
    img: 'https://www.99acres.com/microsite/articles/files/2023/02/condo.jpg',
    roomDetail: '111/A',
    condoName: 'Condo name',
    price: '999',
  },
  {
    id: 6,
    img: 'https://www.99acres.com/microsite/articles/files/2023/02/condo.jpg',
    roomDetail: '222/A',
    condoName: 'Condo name',
    price: '999',
  },
  {
    id: 7,
    img: 'https://www.99acres.com/microsite/articles/files/2023/02/condo.jpg',
    roomDetail: '333/A',
    condoName: 'Condo name',
    price: '999',
  },
  {
    id: 8,
    img: 'https://www.99acres.com/microsite/articles/files/2023/02/condo.jpg',
    roomDetail: '444/A',
    condoName: 'Condo name',
    price: '999',
  },
];

export const sampleReviews = [
  {
    id: '01HQWC9MT86VXQ2QPKB52AZWNS',
    rating: 2,
    agentName: 'Darell Sancias',
    description: 'Ut at dolor quis odio consequat varius.',
  },
  {
    id: '01HQWC9MT996J5AHRZ0FCFTB0B',
    rating: 4,
    agentName: 'Kari Durber',
    description:
      'Vestibulum ante ipsum primis in faucibus orci luctus et ultrices posuere cubilia Curae.',
  },
  {
    id: '01HQWC9MTATQEX7BV613GE9187',
    rating: 1,
    agentName: 'Timmy Philp',
    description:
      'Vestibulum ante ipsum primis in faucibus orci luctus et ultrices posuere cubilia Curae.',
  },
  {
    id: '01HQWC9MTBVXP3Y4CSWHNFWSFT',
    rating: 2,
    agentName: 'Wolfy Oakwood',
    description:
      'Mauris sit amet eros. Suspendisse accumsan tortor quis turpis.',
  },
  {
    id: '01HQWC9MTC7Z9PPMZW30EF3MWB',
    rating: 4,
    agentName: 'Ives Flohard',
    description:
      'Aliquam augue quam, sollicitudin vitae, consectetuer eget, rutrum at, lorem.',
  },
];

export const imageSlides = [
  {
    url: 'https://images.unsplash.com/photo-1493809842364-78817add7ffb?q=80&w=3570&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D',
  },
  {
    url: 'https://images.unsplash.com/photo-1502672260266-1c1ef2d93688?q=80&w=3000&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D',
  },
  {
    url: 'https://images.unsplash.com/photo-1613575831056-0acd5da8f085?q=80&w=3570&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D',
  },
  {
    url: 'https://images.unsplash.com/photo-1621891333819-00c206ec8994?q=80&w=3570&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D',
  },
  {
    url: 'https://plus.unsplash.com/premium_photo-1674676471417-07f613528a94?q=80&w=3545&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D',
  },
];

export const markersMap = [
  {
    id: 1,
    name: 'The Crest Sukumvite 34',
    position: { lat: 13.7251833, lng: 100.576598 },
  },
  {
    id: 2,
    name: 'Kyen Sansiri',
    position: { lat: 13.7250967, lng: 100.5769418 },
  },
  {
    id: 3,
    name: 'The Estelle Phrom Phong',
    position: { lat: 13.728581, lng: 100.5704874 },
  },
  {
    id: 4,
    name: 'The Medison',
    position: { lat: 13.7290801, lng: 100.5715923 },
  },
];
